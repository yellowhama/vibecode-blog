...
| 한글 | 깨질 수 있다 | 안 깨진다 |
| 수정 | 처음부터 다시 | 원하는 부분만 |
| 스타일 변경 | 처음부터 다시 | 말 한 마디 (AI가 태그 리서치) |
...
**Windows / Linux 사용자:** VS Code 또는 Cursor에서 Extensions → "Pencil" 검색 → 설치. 이메일 인증 후 바로 쓸 수 있다.

**Mac 사용자:** [pencil.dev](https://pencil.dev)에서 데스크탑 앱 다운로드. 설치하면 Pencil MCP가 자동으로 설치된다. Claude Code와 자동 연결. VS Code/Cursor 확장도 동일하게 사용 가능.
...
조립하면 레이아웃과 카피가 바로 잡힌다. 스톡 이미지도 자동으로 채워진다. 같은 카피로 스타일만 다르게 뽑은 두 가지 버전이다.

![조립 직후 상태 — 변형 A: 모던 클린](images/mVhCf.png)
*변형 A — 모던 클린 블루. 히어로 + USP 카드 3개.*

![조립 직후 상태 — 변형 B: 프리미엄 다크](images/JghDX.png)
*변형 B — 프리미엄 다크 골드. 같은 카피, 다른 태그.*
...
Pencil에는 디자인 스타일 태그가 200개 넘게 내장돼 있다. 태그를 조합하면 컬러부터 폰트, 간격까지 전부 자동으로 나온다. 근데 사람이 이 태그를 외울 필요는 없다. AI한테 리서치시키면 된다.
...
한 줄이면 된다. AI가 Pencil의 스타일 태그를 리서치해서, 맞는 컬러, 폰트, 간격을 찾아 적용한다.

![스칸디 리서치 적용 — 풀블리드 히어로 + 딥새도 카드 + 그린 컬러바](images/jI9TP.png)
*스칸디 웰니스 — 풀블리드 히어로, 그린 스펙바, 넘버링 카드.*

![럭셔리 골드 — Cormorant Garamond + Inter, 다크+골드](images/2GznF.png)
*럭셔리 골드 — Cormorant Garamond 세리프, 다크 배경, 골드 악센트.*

![럭셔리 골드 리서치 적용 — 풀블리드 + 지그재그 레이아웃 + 골드 스펙 바](images/3klQt.png)
*럭셔리 리서치 적용 — 지그재그 레이아웃, 풀블리드 히어로, 골드 스펙바.*
...
*템플릿 C — Apple 스타일, 다크 풀와이드 1440px.*

![SaaS 랜딩 템플릿 — 라이트, 그라디언트 히어로, 1440px](images/szMa3.png)
*템플릿 D — SaaS 랜딩, 그라디언트 히어로, 라이트 테마.*

여기서 한 걸음 더. 780px 마켓플레이스 규격을 벗어나서, 1440px 웹페이지로 만들면 이렇게 된다.

![1440px Apple-style 웹페이지 — 풀블리드 히어로 + 지그재그 + 스펙바 + CTA](images/6OujP.png)
*v3 — 1440px 데스크탑 웹페이지. 풀블리드 과수원 히어로, 3컬럼 USP, 지그재그 피처, 다크 스펙바, 세이지 CTA. 40분 만에 나온 결과물이다.*

Section 1에서 말한 "애플 아이폰 소개 페이지"가 이거다. 제품 사진, 기능 설명, 구매 버튼. 코드로 만든 웹페이지인데, 상세페이지다.
...
요구사항:
1. 세로 레이아웃, 가로 780px(마켓플레이스) 또는 1440px(웹페이지)
...
사람이 태그를 외울 필요는 없다. 원하는 느낌을 말로 하면 AI가 태그를 리서치해서 적용한다.

| 이렇게 말하면 | AI가 찾는 태그 | 어울리는 제품 |
|--------------|---------------|-------------|
| "따뜻하고 자연스럽게" | japanese, minimal, cream, organic, warm, soft-corners | 식품, 뷰티, 라이프스타일 |
| "깔끔하고 신뢰감 있게" | modern, clean, blue-accent, whitespace, webapp, sharp-corners | IT, SaaS, 전자제품 |
| "고급스럽게" | luxury, dark-mode, gold-accent, serif, elegant, premium | 프리미엄, 명품, 고가 |
| "에너지 넘치게" | bold, neon, dark-mode, electric, vibrant, rounded | 피트니스, 게임, 스트리트 |
| "부드럽고 안정적으로" | scandinavian, pastel, calm, soft-corners, friendly, organic | 웰니스, 키즈, 홈 |

5개로 부족하면 느낌을 더 구체적으로 말하면 된다. "일본 미니멀인데 따뜻한 톤으로" 같은 식으로. AI가 200개 태그 중에서 맞는 걸 골라준다.
