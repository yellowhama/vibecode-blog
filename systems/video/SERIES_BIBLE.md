...
**로그라인**
코딩 모르는 사람도 AI로 제대로 만들 수 있다 — Vee가 그 방법을 찾아가는 여정을 그린 2D 애니메이션.
...
Veritasium 방식을 채택한다:
1. **오해를 먼저 보여준다** — "AI한테 시키면 딸깍이지?"
2. **왜 틀린지 체감시킨다** — 질이 낮거나, 예상과 다르거나, 왜 이렇게 나왔는지 모른다
3. **진짜 답을 발견하게 한다** — 시청자가 답을 스스로 깨닫는 느낌

**교육 공식**
```
[시청자가 믿는 것] → [틀렸다는 증거] → [왜 그런지 메커니즘] → [올바른 방법 시연] → [오늘 바로 쓸 도구/템플릿/행동]
```
6-세그먼트 매핑: Hook+Misconception=착각, The_Crack=증거, Core전반=메커니즘, Core후반=시연, Reframe+Outro=도구
...
**그들의 고통**
- "질이 낮다 — 뭔가 나오긴 하는데 이상하다"
- "예상과 다르다 — 원한 게 이게 아닌데"
- "왜 이렇게 나왔는지 모른다 — 설명할 수 없다"
- "고칠 수 없다 — 코드를 못 읽으니 어디를 고쳐야 하는지 모른다"
- "AI가 매번 다른 답을 줘"
...
- EP01: 요리 = 바이브코딩 (레시피 없이 전자레인지 → 뭔가 나오지만 원한 게 아님, AI = 요리사인데 레시피를 안 주면 즉흥)
...
| 세그먼트 | 시간 | 길이 | 용도 | 비주얼 | 기법 출처 |
|----------|------|------|------|--------|-----------|
| **Hook** | 0:00-0:25 | 25s | 감정 셋업 + open loop | 강렬한 오프닝 이미지 또는 Vee 리액션 | PolyMatter 역설 훅 |
| **Misconception** | 0:25-1:25 | 60s | Vee의 잘못된 가정 → 실패 | Vee가 "당연히 되겠지" → 터짐 (빨간 색조) | Veritasium 오해-먼저 |
| **The Crack** | 1:25-2:15 | 50s | 인지부조화 — "근데 이렇게 하면?" | 전환 장면, 색조 변화 (빨강→중립) | Veritasium |
| **Core** | 2:15-5:45 | 210s | 올바른 방법, 비주얼 메타포, 정보 전달 | 화이트보드 공간, 다이어그램 (초록 색조) | Wendover 점진적 빌드 |
| **Reframe** | 5:45-6:45 | 60s | "사실 더 큰 그림은..." + 패러다임 전환 | 줌아웃, 시스템 뷰 | PolyMatter 미드포인트 리프레임 |
| **Outro+CTA** | 6:45-7:30 | 45s | Vee eureka + 행동 + 다음 에피소드 떡밥 | Vee 리액션 + 시리즈 엔드카드 | — |
...
- 에피소드당 최대 10번
- 각 1-2초 (절대 3초 넘지 않음)
- 배치: Hook 직후 1-2회 + Misconception 중 2회 + The Crack 1-2회 + Core 중 3-4회 + Outro 1회
...
| EP | 제목 | 주제 | 필라 | 시청 후 행동 |
|----|------|------|------|------------|
| 01 | "바이브코딩이 뭔가?" | 정의 + 도구 + 시작법 + 현실 | AI기초 | 도구 설치 + 첫 프롬프트 |
| 02 | "Spec-Driven Dev가 뭔대" | 스펙 + CLAUDE.md + 템플릿 | AI기초 | 3줄 스펙 작성 |
| 03 | "AI가 만든 코드를 어떻게 읽나" | 코드 읽기 (비개발자용) | AI기초 | AI에게 "설명해줘" 프롬프트 |
| 04 | "왜 매번 다른 걸 만드나" | 컨텍스트 관리, SSOT | AI기초 | .cursorrules 세팅 |
| 05 | "바이브코더 5대 실수" | 종합 (EP01-04 요약) | AI기초 | 체크리스트 다운로드 |
| 06 | "10분만에 AI 에이전트" | 핸즈온 빌드 | 실전 | 에이전트 하나 만들기 |
| 07 | "AI는 왜 헛소리를 하나" | 할루시네이션 메커니즘 | AI기초 | 팩트체크 프롬프트 |
| 08 | "프롬프트 실전 가이드" | 좋은/나쁜 프롬프트 비교 | 실전 | 프롬프트 템플릿 5개 |
| 09 | "이 영상은 AI가 만들었다" | BTS 파이프라인 공개 | 비하인드 | 파이프라인 이해 |
| 10 | "좌절에서 시스템으로" | 시즌 마무리 | AI기초 | 개인 시스템 정리 |
...
**Actionable Takeaway 규칙**
매 에피소드 끝에 시청자가 **오늘 바로 실행할 수 있는 행동 1개** 필수.
- "이해했다"가 아니라 "해봤다"가 목표
- Outro_CTA 세그먼트에서 구체적 행동을 제시한다
- 예: "도구 설치해봐", "3줄 적어봐", "AI에게 이 프롬프트를 쳐봐"
