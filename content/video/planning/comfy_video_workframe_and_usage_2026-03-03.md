...
- 기획 문서: `content/video/planning/*` (shot 목적, 액션, 아트 가이드)

### Video 트랙 출력(Downstream)
- 렌더 산출물: `content/video/output/renders/<run_id>/*`
...
### 스크립트
- 배치 렌더러: `content/video/scripts/comfy_batch_render.py`
- Comfy 실행 래퍼: `content/video/scripts/run_comfy_server.sh`
- 배치 실행 래퍼: `content/video/scripts/run_batch_render.sh`
- 블로그 prepro 생성기: `content/video/pipeline/scripts/build_blog_to_video_prepro.py`
- prepro->shot manifest 변환기: `content/video/pipeline/scripts/build_shot_manifest_from_prepro.py`
- 큐 녹음 합본기: `content/video/pipeline/scripts/assemble_voiceover_from_cues.py`
- 자동 TTS 생성기: `content/video/pipeline/scripts/generate_tts_from_prepro.py`
- TTS 환경 점검기: `content/video/pipeline/scripts/tts_env_check.py`
- 오디오 후처리 모듈: `content/video/pipeline/scripts/audio_postprocess.py`
- TTS 백엔드 어댑터: `content/video/pipeline/scripts/tts_backends/*.py`
- manifest 기반 키프레임 sync: `content/video/pipeline/scripts/sync_manifest_keyframes_to_comfy_input.py`
- YouTube 패키저: `content/video/pipeline/scripts/package_for_youtube.py`
- E2E 오케스트레이터: `content/video/pipeline/scripts/run_end_to_end_video_pipeline.py`
- Blog->Video 오케스트레이터: `content/video/pipeline/scripts/run_blog_to_video_pipeline.py`

### 데이터 계약 파일
- 기준선 바인딩: `content/video/pipeline/bindings/hunyuan_i2v_c_test_bindings.json`
- 기준선 매니페스트(Act1 Scene1+2): `content/video/pipeline/manifests/phase1_act1_i2v_hunyuan_baseline_manifest.json`
- 매니페스트 샘플(v2): `content/video/pipeline/manifests/shot_manifest_template_v2.json`

### 워크플로우 레퍼런스
- 기준선 워크플로우: `content/video/workflows/api/hunyuan_i2v_c_test_api.json`
- 비교 워크플로우: `content/video/workflows/api/wan_*.json`
- 실제 실행용은 `content/video/workflows/api/*.json`에 API 포맷 파일 배치 필요
...
```bash
python3 /mnt/e/vibecode-blog/content/video/pipeline/scripts/run_blog_to_video_pipeline.py \
...
3) `i2v` 모드면 샷별 키프레임(`Sxx_xx_keyframe.png`)을 `content/video/assets` 또는 `content/video/keyframes`에 준비
...
python3 /mnt/e/vibecode-blog/content/video/pipeline/scripts/run_blog_to_video_pipeline.py \
  ...
  --target-duration-sec 120 \
  --render \
  --workflow /mnt/e/vibecode-blog/content/video/workflows/api/hunyuan_i2v_c_test_api.json \
  --bindings /mnt/e/vibecode-blog/content/video/pipeline/bindings/hunyuan_i2v_c_test_bindings.json \
...
python3 /mnt/e/vibecode-blog/content/video/pipeline/scripts/run_blog_to_video_pipeline.py \
  ...
  --max-shots 12 \
  --target-duration-sec 120 \
  --recordings-dir /mnt/e/vibecode-blog/content/video/preproduction/<prepro_id>/recordings \
  --render \
  --workflow /mnt/e/vibecode-blog/content/video/workflows/api/hunyuan_i2v_c_test_api.json \
  --bindings /mnt/e/vibecode-blog/content/video/pipeline/bindings/hunyuan_i2v_c_test_bindings.json
```
TTS 백엔드 상태 점검:
```bash
python3 /mnt/e/vibecode-blog/content/video/pipeline/scripts/tts_env_check.py \
  --out /mnt/e/vibecode-blog/content/video/output/logs/tts_env_check.json
```
...
### 0) Comfy 서버 실행
```bash
bash /mnt/e/vibecode-blog/content/video/scripts/run_comfy_server.sh
```
CLI 자동화/장시간 배치 권장 모드(파이프 단절 방지):
```bash
bash /mnt/e/vibecode-blog/content/video/scripts/run_comfy_server.sh --detach
```

### 1) API 워크플로우 준비
- Comfy UI에서 대상 워크플로우를 연 뒤 `File -> Export (API)`
- 기준선 저장 경로: `content/video/workflows/api/hunyuan_i2v_c_test_api.json`
...
### 3) 드라이런 검증
```bash
python3 /mnt/e/vibecode-blog/content/video/scripts/comfy_batch_render.py \
...
### 4) 실제 렌더 실행
```bash
python3 /mnt/e/vibecode-blog/content/video/pipeline/scripts/sync_manifest_keyframes_to_comfy_input.py \
  --manifest /mnt/e/vibecode-blog/content/video/pipeline/manifests/phase1_act1_i2v_hunyuan_baseline_manifest.json
...
### 5) 산출물/로그 확인
- 렌더 결과: `content/video/output/renders/<project>_<timestamp>/<shot_id>/...`
- 실행 로그: `content/video/output/renders/<project>_<timestamp>/render_log.json`

### 6) 학습 루프 실행 (지속 개선)
렌더 직후 아래 분석기를 실행해 `실패 유형/원인/다음 실험안`을 자동 기록합니다.
```bash
python3 /mnt/e/vibecode-blog/content/video/pipeline/scripts/learn_from_run.py \
  --run-dir /mnt/e/vibecode-blog/content/video/output/renders/<run_id> \
...
- `content/video/pipeline/learning/experiment_history.jsonl`

### 7) E2E 실행 (렌더 + QA + YouTube 패키지)
```bash
python3 /mnt/e/vibecode-blog/content/video/pipeline/scripts/run_end_to_end_video_pipeline.py \
  --manifest /mnt/e/vibecode-blog/content/video/pipeline/manifests/phase1_act1_i2v_hunyuan_baseline_manifest.json \
  --workflow /mnt/e/vibecode-blog/content/video/workflows/api/hunyuan_i2v_c_test_api.json \
  --bindings /mnt/e/vibecode-blog/content/video/pipeline/bindings/hunyuan_i2v_c_test_bindings.json \
...
- 기본 search dir: `content/video/assets`, `content/video/keyframes`
- 필요 시 `--keyframe-search-dir`를 반복 지정해 경로를 추가합니다.
- `--checklist-strict`를 쓰면 YouTube 업로드 필수 자동 게이트 FAIL 시 패키징 단계가 즉시 실패합니다.

생성 패키지:
- `content/video/output/youtube_packages/<run_id>/final/*`
- `content/video/output/youtube_packages/<run_id>/thumbnails/*`
- `content/video/output/youtube_packages/<run_id>/metadata/youtube_metadata.json`
- `content/video/output/youtube_packages/<run_id>/metadata/youtube_upload_checklist.json`
- `content/video/output/youtube_packages/<run_id>/metadata/youtube_upload_checklist.md`
...
6. **[NEW] 스크린샷 기반 시각 검증 게이트 (Silent Noise Generation):**
   ...
   - **실제 확인 (2026-03-04):**
     ...
     - *로그:* `content/video/output/renders/phase1_act1_i2v_20260304_094143/render_log.json`
     - *로그 요약:* `total=5, ok_or_dry=5, errors=0`
     - *스크린샷 증거:* `content/video/output/renders/phase1_act1_i2v_20260304_094143/qa_frames/*.jpg`
...
```bash
python3 /mnt/e/vibecode-blog/content/video/pipeline/scripts/validate_phase7_postproduction.py --keep-artifacts
```
...
ffmpeg -y -f lavfi -i "sine=frequency=180:sample_rate=24000:duration=30" \
  ...
  /mnt/e/vibecode-blog/content/video/output/logs/act1_phase7_bgm.wav
...
python3 /mnt/e/vibecode-blog/content/video/pipeline/scripts/run_end_to_end_video_pipeline.py \
  ...
  --twitter-queue /mnt/e/vibecode-blog/content/twitter_queue/2026-w10-act1.json \
  --title "FDD Phase1 Act1 | Claymation Story" \
  --voiceover /mnt/e/vibecode-blog/content/video/preproduction/phase1_act1_ko_20260305_040631/voiceover_master.wav \
  --bgm /mnt/e/vibecode-blog/content/video/output/logs/act1_phase7_bgm.wav \
  --transition fade \
