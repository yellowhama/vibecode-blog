...
1. **Plan (`twitter-plan.md`)**: Analyze the raw sources mapping them to the narrative structure defined in `branding/narrative.md` and `systems/twitter/strategy/STRATEGY.md`. Design Thread beats.
...
3. **Queue (`twitter-queue.md`)**: Map the approved draft into a JSON payload and place it into `systems/twitter/queue/YYYY-wWW.json`.
